 ...
 * @author omelnyk
 */
public class Pattern01DamageTest extends BaseBattleTest implements IModelCallback {

    static final Logger logger = Logger.getLogger(Pattern01DamageTest.class.getName());
...
//    private boolean useAreas = false;
    ...
    private int[][] hpValues = {{150, 0}, {150, 0}, {70, 0}};
...
//        loader.load(null);
        ...
        loader.load("config/bt_ptrn1.json"); // 7 zombies in my car       
        return loader;
    }

    private Boolean checkIfMyUnitsHaveDefinedHP() {
        for (BMFractionEntity curEntity : field.getAllEntities()) {
            if ((curEntity.innerType().contains(BMEntityType.unit)) && (curEntity.getFraction() == 1)
                    && (curEntity.getIdentifier().startsWith(myCar.getIdentifier()))) {
...
//           System.out.println("myUnitsCount : " + myUnitsCount + "; enemyUnitsCount : " + enemyUnitsCount);


        }
//        if (getUseResultOutput()) {
//            System.out.println("\nframe: " + engine.getTimeTicks());
...
//        }
    ...
    public void executeEndLoop() {
        ...
        Assert.assertTrue("My car is destroyed", isMyCarDestroyed);
        Assert.assertEquals("My units are alive and their count is ", 3, myUnitsCountGlobal);
        Assert.assertTrue("Eney car is destroyed", isEnemyCarDestroyed);
        Assert.assertTrue("checkIfMyUnitsHaveDefinedHP", checkIfMyUnitsHaveDefinedHP());
    }
